...
## Verbatim Content (Part 360 of 991)
...
---[FILE: databricks_uc_registry_messages.proto]---
Location: mlflow-master/mlflow/protos/databricks_uc_registry_messages.proto
...
message RegisteredModel {
  ...
  optional string deployment_job_id = 8;

  // Deployment job state for this model.
  optional DeploymentJobConnection.State deployment_job_state = 9;
}
...
message ModelVersion {
  ...
  optional string model_id = 16;

  // Optional parameters for the model.
  repeated ModelParam model_params = 17;

  // Optional metrics for the model.
  repeated ModelMetric model_metrics = 18;
...
enum ModelVersionLineageDirection {
  UPSTREAM = 0;
  DOWNSTREAM = 1;
}

message ModelVersionLineageInfo {
  repeated Entity entities = 1;
  optional ModelVersionLineageDirection direction = 2 [(validate_required) = true];
}

message EmitModelVersionLineageRequest {
  option (scalapb.message).extends = "com.databricks.rpc.RPC[EmitModelVersionLineageResponse]";
  // Name of the registered model
  optional string name = 1 [(validate_required) = true];
  ...
  optional ModelVersionLineageInfo model_version_lineage_info = 3 [(validate_required) = true];
  ...
  optional Securable securable = 4;
}

message EmitModelVersionLineageResponse {}

message IsDatabricksSdkModelsArtifactRepositoryEnabledRequest {
  option (scalapb.message).extends = "com.databricks.rpc.RPC[IsDatabricksSdkModelsArtifactRepositoryEnabledResponse]";
}

message IsDatabricksSdkModelsArtifactRepositoryEnabledResponse {
  optional bool is_databricks_sdk_models_artifact_repository_enabled = 1 [(validate_required) = true];
}
```
