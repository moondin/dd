...
## Verbatim Content (Part 86 of 991)
...
---[FILE: override-predict.ipynb]---
Location: mlflow-master/docs/docs/classic-ml/traditional-ml/tutorials/creating-custom-pyfunc/notebooks/override-predict.ipynb
...
---[FILE: index.mdx]---
Location: mlflow-master/docs/docs/classic-ml/traditional-ml/tutorials/creating-custom-pyfunc/part1-named-flavors/index.mdx
...
learning libraries. For instance, the _spark-ml_ package, despite producing diverse model types such as `Pipeline`, `LogisticRegressionModel`, or
`RandomForestModel`, all fall under the <APILink fn="mlflow.spark">`Spark`</APILink> flavor umbrella. This abstraction ensures that, irrespective of the model's foundational structure,
...
loaded <APILink fn="mlflow.pyfunc">`pyfunc`</APILink> for inference deployment. Each flavor prescribes a `predict` method behavior, ensuring a consistent yet
somewhat rigid format.

To understand these constraints, consider the <APILink fn="mlflow.sklearn">`sklearn`</APILink> flavor as an example. The diagram below delineates its
implementation, highlighting the APIs and serialization methods MLflow has standardized:

<figure className="center-div" style={{ width: "1024px", maxWidth: "100%", textAlign: "center" }}>
  ![MLflow's sklearn implementation](/images/guides/introductory/creating-custom-pyfunc/anatomy_of_a_model_flavor.svg)
  <figcaption>A depiction of the standards adopted by MLflow's sklearn flavor</figcaption>
</figure>
...
However, there's a silver lining. MLflow offers the flexibility to craft a custom `pyfunc` by extending the foundational <APILink fn="mlflow.pyfunc.PythonModel">`PythonModel`</APILink>
...
To delve deeper into these functionalities, let's examine the core structure of an MLflow <APILink fn="mlflow.models.model.Model">`Model`</APILink>.
...
<figure className="center-div" style={{ width: 480, maxWidth: "100%", textAlign: "center" }}>
  ![Components of a Model in MLflow](/images/guides/introductory/creating-custom-pyfunc/anatomy-of-a-model.svg)
...
6. **PyFunc Signature**:
   To ensure seamless deployment and inference, MLflow wraps the model in a standardized _pyfunc_ interface. This interface defines the expected input and output formats, ensuring consistency.
...
<figure className="center-div" style={{ width: 1024, maxWidth: "100%", textAlign: "center" }}>
  ![Components of a Model in the MLflow UI](/images/guides/introductory/creating-custom-pyfunc/model-components-in-ui.svg)
  <figcaption>Model components seen in the MLflow artifact viewer</figcaption>
</figure>
