...
## Verbatim Content (Part 815 of 1290)
...
---[FILE: search_suggestion.test.cjs]---
Location: zulip-main/web/tests/search_suggestion.test.cjs
...
test("dm_suggestions", ({override, mock_template}) => {
    ...
    suggestions = get_suggestions(query);
    expected = ["is:unread sender:ted@zulip.com"];
    assert.deepEqual(suggestions.strings, expected);
...
test("sent_by_me_suggestions", ({override, mock_template}) => {
    ...
    suggestions = get_suggestions(query);
    expected = ["is:starred sender:myself@zulip.com"];
    assert.deepEqual(suggestions.strings, expected);
});
...
---[FILE: server_events.test.cjs]---
Location: zulip-main/web/tests/server_events.test.cjs
