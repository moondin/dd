...
## Verbatim Content (Part 932 of 1290)
...
Location: zulip-main/zerver/migrations/0001_initial.py
Signals: Django
...
def set_up_fts_indexes(apps: StateApps, schema_editor: BaseDatabaseSchemaEditor) -> None:
    # Duplicated in 0001_squashed_0569.py
    with connection.cursor() as cursor:
        try:
            ...
            # running with a hosted PostgreSQL which we can't install
            # our stemming/stopword dictionaries in; warn and carry
            # on.  For local PostgreSQL hosts, this is unexpected and
            # we raise the error.
            if (
                settings.REMOTE_POSTGRES_HOST != ""
                and e.__cause__
                and "/tsearch_data/" in str(e.__cause__)
            ):
                logging.warning(
                    "Remote PostgreSQL server %s is not configured with Zulip's improved stemming dictionaries; search quality will be degraded.",
                    settings.REMOTE_POSTGRES_HOST,
                )
                cursor.execute(
                    """
...
class Migration(migrations.Migration):
    ...
    operations = [
        ...
        migrations.RunPython(set_up_fts_indexes),
    ]
```
